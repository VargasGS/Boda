...
console.log(Nombre,Asistencia,Descripcion)

    req.getConnection((err, conn) => {
        

      const resul=  conn.query("CALL confirmarAsistencia('" + Nombre + "','" + Asistencia + "','" + Descripcion + "')", (err, invitados) => {
            if (err) {
              console.log("error")
              res.status(404);
              let er = "Ya se ha realizado la confirmación anteriormente";
